...
- **Scan Dependencies**: lists all `dependencies`, `devDependencies`, `peerDependencies` and `optionalDependencies` per folder. The full-width search below the scan button filters `package.json` folder and file paths; the full package manager can also be opened directly without choosing a package first.
- **Root-aware compatibility**: a nested library `peerDependency` is treated as compatible when its range fully contains the root range. For example, root `~21.2.0` and peer `^21.2.0` are not reported as a conflict. Filters can show only real conflicts or additionally expose compatible textual differences.
- **Status-rich package trees**: both the sidebar and full editor show declared versions, dependency types, conflict summaries and optional compatible-difference indicators. Every scanned `package.json` has a direct action that opens the file in VS Code.
- **Full-size package editor**: click a package, use the sidebar action, or run **AngularPad: Open Package Manager** to open or reuse the editor tab beside the active editor. The first package is selected automatically, the detail pane uses the available height, and package occurrences provide both **Select all** and **Deselect all** actions.
